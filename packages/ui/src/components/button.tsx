...
interface ButtonProps extends useRender.ComponentProps<"button"> {
  ...
  /** 显示 loading 态：禁用按钮、隐藏内容、居中显示 Spinner。 */
  loading?: boolean;
}

function Button({ className, variant, size, render, loading, children, ...props }: ButtonProps) {
    ...
    props: mergeProps<"button">(defaultProps, props, {
      children: loading ? (
        <>
          ...
          <Spinner data-slot="button-loading-indicator" className="absolute" aria-label="Loading" />
        </>
      ) : (
        children
      ),
